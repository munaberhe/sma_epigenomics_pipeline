...
make_volcano <- function(dmr_df, title, ylim_max) {
  dmr_df <- dmr_df %>%
    mutate(
      ...
      log_p     = -log10(pValue),
      colour    = case_when(
        meth_diff < -0.20 & pValue < 0.05 ~ "Hypo",
        meth_diff >  0.20 & pValue < 0.05 ~ "Hyper",
        TRUE ~ "NS"
      )
    )
  ...
  if (has_hits && nrow(top10) > 0) {
    p <- p + geom_text_repel(
      data=top10,
      aes(x=meth_diff*100, y=pmin(-log10(pValue), ylim_max),
          label=sprintf("%s:%d", seqnames, start)),
      size=2.5, max.overlaps=15, colour="#1F3B5B"
    )
  }
  ...
  p <- p +
    labs(title=title,
         ...
         caption=sprintf("Thresholds: |Δβ| > 20%%, p < 0.05 | Hypo=%d, Hyper=%d",
                         n_hypo, n_hyper)) +
...
plots <- lapply(seq_along(CONTRASTS), function(i) {
  ...
  d  <- dmr_list[[i]]
  if (is.null(d)) return(NULL)
  make_volcano(d, ct$title, ylim_max)
})
