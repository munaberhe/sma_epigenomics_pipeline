...
# Additional QC plots:
# 1. M-bias plots
# 2. Duplication rate bar chart
# 3. Bisulfite conversion efficiency bar chart
# 4. Per-condition CpG methylation violin
...
read_mbias <- function(sample) {
  f <- file.path(BS_DIR,
       paste0(sample, "_bismark.deduplicated.M-bias.txt"))
  lines <- readLines(f)
...
p_mbias <- ggplot(mbias_cpg,
                  aes(x=position, y=pct_methylation,
  ...
  labs(title="M-bias plot — CpG context",
       subtitle=paste0("All 12 samples overlaid. Flat lines = no positional bias.\n",
                       "Spike at read ends = incomplete bisulfite conversion."),
       x="Position in read (bp)", y="% CpG methylation") +
  theme_classic(base_size=11) +
  theme(plot.title=element_text(face="bold"),
        strip.text=element_text(face="bold"))

ggsave(file.path(OUT, "mbias_CpG_all_samples.pdf"),
       p_mbias, width=10, height=8)
message("Saved: mbias_CpG_all_samples.pdf")
...
dup_df <- do.call(rbind, lapply(SAMPLES, function(s) {
  f <- file.path(BS_DIR,
       paste0(s, "_bismark.deduplication_report.txt"))
  pct_line <- grep("Total number duplicated",
                   readLines(f), value=TRUE)
...
p_dup <- ggplot(dup_df, aes(x=sample, y=dup_rate, fill=condition)) +
  geom_col() +
  geom_text(aes(label=paste0(round(dup_rate,1),"%")),
            vjust=-0.3, size=3) +
  scale_fill_manual(values=GROUP_COLS, name="Condition") +
  scale_y_continuous(limits=c(0, max(dup_df$dup_rate)*1.2)) +
  labs(title="PCR duplication rates per sample",
       subtitle="Consistent rates across conditions indicate no library preparation bias",
       x=NULL, y="Duplication rate (%)") +
  theme_classic(base_size=11) +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=9),
        plot.title=element_text(face="bold"))

ggsave(file.path(OUT, "duplication_rates.pdf"),
       p_dup, width=10, height=5)
message("Saved: duplication_rates.pdf")
...
conv_df <- do.call(rbind, lapply(SAMPLES, function(s) {
  ...
  eff <- round(chh_u * 100 / (chh_u + chh_m), 1)
...
write.csv(conv_df,
          file.path(OUT, "bisulfite_conversion_efficiency.csv"),
          row.names=FALSE)

p_conv <- ggplot(conv_df,
                 aes(x=sample, y=conversion_efficiency_pct,
                     fill=condition)) +
  geom_col() +
  ...
  labs(title="Bisulfite conversion efficiency per sample",
       subtitle="Calculated from CHH context (expected ~0% true methylation in human somatic cells)",
       x=NULL, y="Conversion efficiency (%)") +
  theme_classic(base_size=11) +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=9),
        plot.title=element_text(face="bold"))

ggsave(file.path(OUT, "bisulfite_conversion_efficiency.pdf"),
       p_conv, width=10, height=5)
message("Saved: bisulfite_conversion_efficiency.pdf")

# ── 4. PER-CONDITION METHYLATION VIOLIN ──────────────────────
message("Generating methylation violin plot...")

suppressPackageStartupMessages(library(GenomicRanges))

load_dmr <- function(contrast, prop_col) {
  d <- readRDS(paste0("results/dmr/dmr_", contrast, ".rds"))
  d <- d[d$cytosinesCount >= 6]
  data.frame(methylation=mcols(d)[[prop_col]],
             condition=prop_col)
}
...
violin_df <- rbind(
  data.frame(methylation=local({
    ...
    d <- d[d$cytosinesCount >= 6]; d$proportion1}),
    condition="ASO_CTRL"),
  data.frame(methylation=local({
    d <- readRDS("results/dmr/dmr_ASO_CTRL_vs_Scramble_CTRL.rds")
    d <- d[d$cytosinesCount >= 6]; d$proportion2}),
    condition="Scramble_CTRL"),
  data.frame(methylation=local({
...
p_violin <- ggplot(violin_df,
                   aes(x=condition, y=methylation, fill=condition)) +
  geom_violin(trim=FALSE, alpha=0.8) +
  geom_boxplot(width=0.08, fill="white",
               outlier.size=0.2, outlier.alpha=0.2) +
  scale_fill_manual(values=GROUP_COLS) +
  scale_y_continuous(labels=scales::percent_format(accuracy=1)) +
  labs(title="CpG methylation distribution at high-confidence DMR loci",
       subtitle="cytosinesCount >= 6 | pooled replicates per condition",
...
ggsave(file.path(OUT, "per_condition_methylation_violin.pdf"),
       p_violin, width=8, height=6)
message("Saved: per_condition_methylation_violin.pdf")
