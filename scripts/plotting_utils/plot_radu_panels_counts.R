...
HAS_CONTRAST <- FALSE  # no contrast column in this dataset
if (HAS_CONTRAST) {
...
make_panels <- function(null_label, filename_tag, contrast_key) {
  ...
  # drop NA rows
  d <- d[!is.na(d$n_real) & !is.na(d$n_scrambled), ]
  if (nrow(d) == 0) {
    message("  [skip] no rows or all NA for ", null_label, " / ", contrast_key)
  ...
  pB_data <- d
  pB <- ggplot(pB_data,
               aes(x = window_size_num, y = n_real - n_scrambled,
    ...
    labs(
      ...
      subtitle = paste0("null model: ", null_label),
      x = "Bin/window size (bp)",
      y = "Delta DMRs (real \u2212 scrambled)"
    ) +
    theme_radu

  # Position panel B legend depending on data sign
  if (mean(pB_data$n_real - pB_data$n_scrambled, na.rm = TRUE) < 0) {
