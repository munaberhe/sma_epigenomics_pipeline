...
# Tests whether ASO-specific DMRs are enriched near exon-intron splice junctions
# compared to a matched random background.
# Splice sites extracted from Ensembl GRCh38.109 GTF using awk (protein-coding only)
# stored at data/reference/splice_sites_hg38_protein_coding.bed
...
# Load ASO-specific DMRs
message("Loading ASO-specific DMRs...")
dmrs <- readRDS(DMR_FILE)
dmrs <- dmrs[as.character(seqnames(dmrs)) %in% KEEP_CHRS]
message("  DMRs: ", length(dmrs))

# Load splice sites from BED file
message("Loading splice sites from BED file...")
...
# Distance from each ASO DMR to nearest splice junction
message("Calculating distances: ASO DMRs to nearest splice junction...")
dmr_dist      <- distanceToNearest(dmrs, splice_sites)
dmr_distances <- mcols(dmr_dist)$distance
message("  Median distance: ", median(dmr_distances), " bp")
message("  DMRs within 300bp: ", sum(dmr_distances <= 300),
        " (", round(100*mean(dmr_distances <= 300), 1), "%)")

# Matched random background
message("Generating matched background regions (n=", N_BG, ")...")
chr_sizes <- c(
...
message("  Background median distance: ", median(bg_distances), " bp")

# Wilcoxon test
wtest <- wilcox.test(dmr_distances, bg_distances, alternative="less")
message("\nWilcoxon test (DMRs closer to splice sites than background):")
message("  p-value: ", signif(wtest$p.value, 3))
message("  DMR median: ", median(dmr_distances), " bp")
...
write.csv(results_df,
          file.path(OUT_DIR, "splice_junction_distances.csv"),
          row.names=FALSE)
...
write.csv(summary_df,
          file.path(OUT_DIR, "splice_junction_summary.csv"),
          row.names=FALSE)
...
p <- ggplot(results_df, aes(x=distance, fill=type, colour=type)) +
  ...
  annotate("text", x=350, y=Inf, label="300bp\nbin size",
           hjust=0, vjust=1.5, size=3, colour="grey40") +
  labs(
    title="Distance from ASO-specific DMRs to nearest splice junction",
    subtitle=sprintf(
      "114 ASO-specific DMRs vs %d matched background regions\nWilcoxon p = %s | DMR median = %d bp | Background median = %d bp",
      N_BG, signif(wtest$p.value, 3),
      median(dmr_distances), median(bg_distances)),
    x="Distance to nearest splice junction (bp, log scale)",
    y="Density",
    fill=NULL, colour=NULL,
    caption=if(wtest$p.value < 0.05)
      "ASO-specific DMRs are significantly closer to splice junctions than background" else
      "No significant enrichment near splice junctions"
  ) +
  ...
  theme(
    ...
    plot.caption  = element_text(size=8, face="italic",
                      colour=if(wtest$p.value < 0.05) "#1B4F8A" else "#D94F3D"),
    legend.position="top"
  )

ggsave(file.path(OUT_DIR, "splice_junction_distance_density.pdf"),
       p, width=10, height=6)
