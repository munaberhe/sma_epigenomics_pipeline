...
build_panel <- function(contrast, ann, label_map, panel_letter, panel_title) {
  ...
  if (file.exists(tested_path)) {
    ...
    # fall back: use genome-wide CpG-covered windows from pooled cache
    message("no tested_windows for ", contrast, "; using genome-wide CpG background")
    cache <- readRDS("results/dmr/meth_pooled_cache.rds")
    bg <- cache[[1]]
    bg <- bg[bg$readsN >= 1]
    # subsample to 50k for speed
    if (length(bg) > 50000) { set.seed(42); bg <- bg[sample(length(bg), 50000)] }
  }
