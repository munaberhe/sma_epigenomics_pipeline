...
p_ls <- ggplot(ls_df, aes(x = window_size)) +
  ...
  annotate("text", x = SELECTED_WS * 1.15, y = max(ls_df$n_real, na.rm = TRUE) * 0.8,
           label = paste0("Selected bin size\n(", SELECTED_WS, " bp)"),
           angle = 0, vjust = 1, hjust = 0, size = 3.0, colour = "grey30") +
  scale_colour_manual(values = c("Real" = NAVY, "Scrambled (mean of 9 real partitions)" = GREY),
                      name = NULL) +
  scale_shape_manual(values = c("Real" = 16, "Scrambled (mean of 9 real partitions)" = 1),
                     name = NULL) +
...
p_rc <- ggplot(rc_df, aes(x = window_size)) +
  ...
  scale_colour_manual(values = c("Real" = NAVY, "Scrambled (mean of 20 read-count permutations)" = GREY),
                      name = NULL) +
  scale_shape_manual(values = c("Real" = 16, "Scrambled (mean of 20 read-count permutations)" = 1),
                     name = NULL) +
  scale_x_log10(breaks = c(100, 200, 300, 500, 1000, 2000)) +
  scale_y_log10(labels = scales::comma) +
