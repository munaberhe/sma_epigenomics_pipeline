...
for (ch in chroms) {
  ...
  for (i in seq_len(nrow(chr_candidates))) {
    ...
    perm_pval <- (1 + sum(abs(perm_devs) >= abs(deviation))) / (1 + N_PERM)
...
results_df$perm_pval_adj <- p.adjust(results_df$perm_pval, method="BH")
...
message("Candidates with BH-adjusted perm_pval < 0.05: ", sum(results_df$perm_pval_adj < 0.05, na.rm=TRUE))
