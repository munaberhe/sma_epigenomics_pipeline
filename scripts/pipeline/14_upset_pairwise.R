...
# membership matrix - one row per locus, one col per contrast
...
# sanity check - column sums should approximate DMR counts
...
# compute thesis-critical intersections explicitly
...
# plot - highlight thesis-critical intersections
...
upset(
  ...
  queries = list(
    ...
    list(query=intersects, params=list("ASO_in_VPA","VPA_in_ASO"),
         color="#C0392B", active=TRUE, query.name="Pairwise synergy (both combination contrasts)")
  )
)
grid.text(
...
grid.text(
  paste0(
    "One row per unique non-overlapping locus in the union of all DMR sets ",
    "(n=", format(nrow(mat), big.mark=","), " loci). ",
    "n = 836,595 unique loci. Loci formed by GenomicRanges::reduce()."
  ),
  x=0.65, y=0.948,
