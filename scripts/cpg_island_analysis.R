...
# Load CpG islands from UCSC via AnnotationHub
message("loading CpG island annotations...")
ah <- AnnotationHub()
cpg_islands <- ah[["AH5086"]]  # hg38 CpG islands
seqlevelsStyle(cpg_islands) <- "UCSC"

# Define shores (2kb flanking islands) and shelves (2-4kb)
shore_size  <- 2000
shelf_size  <- 2000
shores <- c(
  GRanges(seqnames(cpg_islands),
          IRanges(start(cpg_islands) - shore_size,
                  start(cpg_islands) - 1)),
  GRanges(seqnames(cpg_islands),
          IRanges(end(cpg_islands) + 1,
                  end(cpg_islands) + shore_size))
)
shelves <- c(
  GRanges(seqnames(cpg_islands),
          IRanges(start(cpg_islands) - shore_size - shelf_size,
                  start(cpg_islands) - shore_size - 1)),
  GRanges(seqnames(cpg_islands),
          IRanges(end(cpg_islands) + shore_size + 1,
                  end(cpg_islands) + shore_size + shelf_size))
)
