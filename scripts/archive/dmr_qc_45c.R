...
# ============================================================
# 5. Genome-wide methylation distribution — run first
# ============================================================
...
GenomeInfoDb::seqlevelsStyle(cpgi) <- 'UCSC'
shores <- reduce(c(flank(cpgi, 2000, start=TRUE),
                    flank(cpgi, 2000, start=FALSE)))

overlap_by_chr <- function(dmrs, features) {
...
results <- do.call(rbind, lapply(seq_along(CONTRASTS), function(i) {
  d <- dmr_list[[CONTRASTS[i]]]
  GenomeInfoDb::seqlevelsStyle(d) <- 'UCSC'
  total    <- length(d)
  cat('Processing', CONTRASTS[i], '(', total, 'DMRs)...\n')
  n_island <- overlap_by_chr(d, cpgi)
  n_shore  <- max(overlap_by_chr(d, shores) - n_island, 0)
