...
# =============================================================================
# plot_radu_panels.R  (polished — style-matched to slides 11-15)
#
# Drop-in replacement for the previous plot_radu_panels.R.
# Reads the same CSVs from the same folder, writes PDFs with the same names
# (radu_panels_<null>.pdf) into the same plots/ subfolder.
#
# Key changes vs the previous version:
#   1. Scrambled dashed lines now visible behind the solid real line
#      - Scrambled x-coordinates offset by a small fraction (visible peek)
#      - Lighter, distinct colour for scrambled (not just dashed)
#   2. Bold A/B panel titles, italic grey caption footer (slide 11/12 style)
#   3. Navy + brick-red palette (replaces teal/blue/pink)
#   4. Inline boxed legend in panel A so it sits inside the plotting area
#   5. NB excluded from coverage panels (count is constant w.r.t. window size)
#      - this matches what your old script's caption said but didn't enforce
# =============================================================================

# ---- Where to read from ------------------------------------------------------
# Set TURBO=1 to use the turbo (minDiff=0.2 strict / 0.1 loose) outputs.
# Default = turbo, since that is now your canonical benchmark.
...
# Strict mode + Gaussian-only NF (matches your previous script)
...
# If a `contrast` column exists in the CSVs (e.g. "VPA_effect", "ASO_effect",
# "Combined"), the script will loop over all of them. If not, it falls back
# to a single contrast labelled "ASO_VPA vs ASO_CTRL" (your current data).
...
# Bright versions for REAL, lighter versions for SCRAMBLED (so dashed lines
# remain visible even when sitting directly under solid lines).
col_real   <- c("DMRcaller-B"  = "#2C5F8D",  # navy
                "DMRcaller-NB" = "#7F8C8D",  # neutral grey (rarely shown)
                "DMRcaller-NF" = "#C0392B")  # brick red
col_scram  <- c("DMRcaller-B"  = "#7FB0D3",  # light navy
                "DMRcaller-NB" = "#BDC3C7",
                "DMRcaller-NF" = "#E8A39B")  # light red

shapes_method <- c("DMRcaller-B" = 15, "DMRcaller-NB" = 17, "DMRcaller-NF" = 18)

# Order legend entries: B real, B scrambled, NF real, NF scrambled
key_levels <- c("DMRcaller-B (real)", "DMRcaller-B (scrambled)",
                "DMRcaller-NF (real)", "DMRcaller-NF (scrambled)")
key_cols <- setNames(c(col_real["DMRcaller-B"],  col_scram["DMRcaller-B"],
                       col_real["DMRcaller-NF"], col_scram["DMRcaller-NF"]),
                     key_levels)
# Use a long-dash pattern for scrambled ("longdash" = bigger dashes than
# "dashed") so the pattern reads clearly at print size, especially when the
# scrambled line sits close to the real one.
...
# ---- Plot one (null model, contrast) pair ----------------------------------
make_panels <- function(null_label, filename_tag, contrast_key) {
  d <- df_s[df_s$scramble_method == null_label &
            ...
            df_s$method          != "DMRcaller-NB", ]   # NB excluded (constant)
  if (nrow(d) == 0) {
  ...
  # Long format with BIGGER offset on x for the scrambled series so the
  # dashed line is clearly separated even when y-values are identical.
  # 5% offset on a log axis is visually clear without distorting the data.
  d_long <- rbind(
    data.frame(method = d$method,
               x      = d$window_size_num * 0.95,         # real shifted LEFT 5%
               y      = d$cov_real_mb,
               series = "real",
               stringsAsFactors = FALSE),
    data.frame(method = d$method,
               x      = d$window_size_num * 1.05,         # scrambled shifted RIGHT 5%
  ...
  pA <- ggplot(d_long,
    ...
    # Thick dashed scrambled lines so the dash pattern is visible at print size
  ...
  # ---- Panel B: real - scrambled ------------------------------------------
  pB_data <- d  # already excludes NB
  pB_data$method_lab <- paste0(pB_data$method, " (delta)")

  # Single combined legend for panel B (force colour + shape into one guide)
  pB <- ggplot(pB_data,
    ...
    scale_colour_manual(values = col_real, name = NULL,
  ...
  caption_text <- switch(null_label,
    ...
    "Read count permutation" = paste0(
      "Scrambled > real (negative delta) is the expected result \u2014 ",
      "read count permutation is a more stringent null than label-swap. ",
      "Real DMRs must exceed a higher false-positive floor."),
