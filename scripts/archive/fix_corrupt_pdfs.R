...
for (contrast in contrasts) {
  cat('Regenerating plots for:', contrast, '\n')
  anno_df <- read.csv(file.path(OUT, paste0(contrast, '_annotated.csv')))

  # Rebuild anno object from CSV
  ...
  # Annotation bar — cairo_pdf for compatibility
  cairo_pdf(file.path(OUT, paste0(contrast, '_annotation_bar.pdf')),
            width=10, height=6)
  print(plotAnnoBar(anno, title=paste0('Genomic Feature Distribution — ', contrast)))
  dev.off()
  cat(' annotation_bar done\n')

  # TSS distance
  cairo_pdf(file.path(OUT, paste0(contrast, '_TSS_distance.pdf')),
            width=8, height=5)
  print(plotDistToTSS(anno, title=paste0('Distance to TSS — ', contrast)))
  dev.off()
  cat(' TSS_distance done\n')
}
