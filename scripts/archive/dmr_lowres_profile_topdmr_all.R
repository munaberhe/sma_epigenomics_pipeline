...
for (ct in contrasts) {
  ...
  profiles_gr <- GRangesList(setNames(
    ...
    c(g1, g2)))
  out_pdf <- file.path(out_dir, paste0("low_resolution_topdmr_", tag, "_chr1_50kb.pdf"))
  pdf(out_pdf, width=10, height=5)
  par(mar=c(4,4,3,1)+0.1)
  plotMethylationProfile(
    methylationProfiles = profiles_gr,
    autoscale = FALSE, labels = NULL,
    title = paste0("CG methylation chr1:224-236Mb (50kb bins)\n", g1, " vs ", g2),
    col = colours, pch = c(15,16), lty = c(1,2))
  dev.off()
