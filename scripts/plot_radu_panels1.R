...
key_cols <- setNames(c(col_real["DMRcaller-B"],  col_scram["DMRcaller-B"],
                     ...
                     key_levels)
key_lty  <- setNames(c("solid", "dashed", "solid", "dashed"), key_levels)
...
make_panels <- function(null_label, filename_tag, contrast_key) {
  ...
  # Long format with offset on x for the scrambled series so dashes peek out
  d_long <- rbind(
    data.frame(method = d$method,
               x      = d$window_size_num * 0.975,        # real shifted slightly LEFT
               y      = d$cov_real_mb,
               series = "real",
               stringsAsFactors = FALSE),
    data.frame(method = d$method,
               x      = d$window_size_num * 1.025,        # scrambled shifted slightly RIGHT
  ...
  pB <- ggplot(pB_data,
    ...
    scale_colour_manual(values = col_real,
