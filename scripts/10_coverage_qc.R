...
make_panel <- function(cond) {
    ...
    ggplot(df_c, aes(x = threshold, y = retention, colour = track)) +
        ...
        labs(title = cond, x = "Coverage threshold (reads per CpG)",
             y = "Fraction of CpGs >= threshold", colour = NULL) +
...
combined <- (panels[[1]] / panels[[2]] / panels[[3]] / panels[[4]]) +
    ...
    plot_annotation(
        ...
        subtitle = "Retention curves - pooled (rust) sits above replicates (blues) when pooling works",
...
ggsave(file.path(out_dir, "coverage_4lines_per_condition.pdf"), combined, width = 8, height = 12)
ggsave(file.path(out_dir, "coverage_4lines_per_condition.png"), combined, width = 8, height = 12, dpi = 200)
write.table(summary_all, file.path(out_dir, "coverage_4lines_per_condition_summary.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)

message("\nNumeric summary across all conditions:")
