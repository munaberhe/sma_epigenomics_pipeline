...
message("Getting TSS coordinates (all autosomes + chrX)...")
txdb  <- TxDb.Hsapiens.UCSC.hg38.knownGene
genes <- genes(txdb)
genes <- genes[!seqnames(genes) %in% c("chrY","chrM") & !grepl("_", as.character(seqnames(genes))) & width(genes) > 1000]
...
message("  DMRs (CG, ASO_CTRL vs Scramble_CTRL): ", length(dmr_aso))
...
pdf(file.path(OUT, "DMR_heatmap_top500_ASO_alone.pdf"), width=8, height=12)
Heatmap(heatmap_mat,
        name="CpG meth",
        col=col_fun,
