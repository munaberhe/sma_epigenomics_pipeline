# parameter_benchmark_helpers_turbo.R
#
# PLAN C: serial real+scrambled execution with 32 cores in ONE SnowParam.
# Identical logic to Plan B helpers, with two differences:
#   - INNER_WORKERS = 32 (was 16)
#   - OUT_DIR has "_turbo" suffix — independent of Plan A/B output
...
OUT_DIR    <- "results/dmr_benchmark_turbo"
CHROM      <- "chr1"
...
load_chr1_data <- function() {
  ...
  list(
    aso_vpa  = load_one(c("ASO_VPA_1",  "ASO_VPA_2",  "ASO_VPA_3")),
...
run_dmrs_one <- function(treat, ctrl, method, ws, kernel="triangular", strict=FALSE, region) {
  th <- get_thresholds(method, strict)
  tryCatch({
    if (method == "noise_filter") {
      computeDMRs(treat, ctrl, regions=region, context="CG", method="noise_filter",
        ...
        minGap=th$minGap, minSize=th$minSize,
        minReadsPerCytosine=4, BPPARAM=BPPARAM_FAST, parallel=TRUE)
    } else if (method == "bins") {
      computeDMRs(treat, ctrl, regions=region, context="CG", method="bins",
        binSize=ws,
        test=th$test, pValueThreshold=th$pval,
        minCytosinesCount=th$minCpG, minProportionDifference=th$minDiff,
        minGap=th$minGap, minSize=th$minSize,
      ...
      computeDMRs(treat, ctrl, regions=region, context="CG", method="neighbourhood",
        test=th$test, pValueThreshold=th$pval,
        minCytosinesCount=th$minCpG, minProportionDifference=th$minDiff,
        minGap=th$minGap, minSize=th$minSize,
        minReadsPerCytosine=4, BPPARAM=BPPARAM_FAST, parallel=TRUE)
...
run_dmrs_pair_parallel <- function(treat_real, ctrl_real, treat_scr, ctrl_scr,
  ...
  t_start <- Sys.time()
  message("  [turbo] Real branch (32 workers)...")
  s <- Sys.time()
  ...
  message("  [turbo] Real branch done in ", round(t_real), "s")

  message("  [turbo] Scrambled branch (32 workers)...")
  s <- Sys.time()
  out_scr <- run_dmrs_one(treat_scr, ctrl_scr, method, ws, kernel, strict, region)
  t_scr <- as.numeric(difftime(Sys.time(), s, units = "secs"))
  message("  [turbo] Scrambled branch done in ", round(t_scr), "s")
...
make_result_row <- function(pair, method, ws, mode_lab, kernel, null_name) {
  ...
  ratio <- if (!is.na(n_real) && !is.na(n_scr) && n_scr > 0) round(n_real / n_scr, 2)
           else if (!is.na(n_real) && !is.na(n_scr) && n_scr == 0) Inf else NA_real_
  ...
  data.frame(method=method, window_size=ws, mode=mode_lab, kernel=kernel,
    ...
    t_wall_secs=round(pair$t_wall, 1),
    error_real=err_real, error_scr=err_scr, stringsAsFactors=FALSE)
}
...
run_benchmark_loop <- function(aso_vpa, aso_ctrl, aso_vpa_scr, aso_ctrl_scr, null_name) {
  ...
  for (method in METHODS) {
    for (ws in WINDOW_SIZES) {
      ...
      for (strict in MODES) {
        ...
        if (!is.null(results[[key]])) { message("\n--- [skip cached] ", key, " ---"); next }
        message("\n--- ", mode_lab, " | ", method, " | ws=", ws, " | kernel=", ker, " ---")
        message("  Running real + scrambled SERIALLY (Plan C turbo, 32 workers)...")
        ...
        message("  Cell wallclock (turbo): ", round(pair$t_wall), "s")
  ...
  write.csv(summary_df, file.path(OUT_DIR, paste0("parameter_benchmark_", null_name, ".csv")),
            row.names = FALSE)
