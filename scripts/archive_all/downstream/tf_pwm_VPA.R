...
gr <- readRDS("results/dmr/dmr_Scramble_VPA_vs_Scramble_CTRL.rds")
...
saveRDS(rep, file.path(OUT_DIR, "pwmenrich_report_VPA_5k.rds"))
# filter blacklist
...
write.csv(df, file.path(OUT_DIR, "pwmenrich_top_motifs_VPA_5k.csv"), row.names=FALSE)
# native PWMEnrich plot - top 20
N20 <- min(20, length(rep_named))
cairo_pdf(file.path(OUT_DIR, "pwmenrich_report_VPA_5k.pdf"), width=12, height=max(10,N20*0.65))
plot(rep_named[1:N20])
dev.off()
message("saved: pwmenrich_report_VPA_5k.pdf")
# native PWMEnrich plot - top 10
N10 <- min(10, length(rep_named))
cairo_pdf(file.path(OUT_DIR, "pwmenrich_top10_VPA_5k.pdf"), width=11, height=max(8,N10*0.70))
