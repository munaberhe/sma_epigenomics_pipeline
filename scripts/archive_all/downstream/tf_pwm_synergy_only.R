...
gr_combo <- gr_combo[gr_combo$context=="CG" & as.character(seqnames(gr_combo)) %in% KEEP_CHRS]
...
gr <- gr_combo[countOverlaps(gr_combo,gr_aso)==0 & countOverlaps(gr_combo,gr_vpa)==0]
message("synergy DMRs: ", length(gr))
...
saveRDS(rep, file.path(OUT_DIR, "pwmenrich_report_synergy_only_5k.rds"))
# filter blacklist
...
write.csv(df, file.path(OUT_DIR, "pwmenrich_top_motifs_synergy_only_5k.csv"), row.names=FALSE)
# native PWMEnrich plot - top 20
N20 <- min(20, length(rep_named))
cairo_pdf(file.path(OUT_DIR, "pwmenrich_report_synergy_only_5k.pdf"), width=12, height=max(10,N20*0.65))
plot(rep_named[1:N20])
dev.off()
message("saved: pwmenrich_report_synergy_only_5k.pdf")
# native PWMEnrich plot - top 10
N10 <- min(10, length(rep_named))
cairo_pdf(file.path(OUT_DIR, "pwmenrich_top10_synergy_only_5k.pdf"), width=11, height=max(8,N10*0.70))
plot(rep_named[1:N10])
dev.off()
message("Done: synergy_only")
