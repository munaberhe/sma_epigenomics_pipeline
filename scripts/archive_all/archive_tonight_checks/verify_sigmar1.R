for (cn in c("ASO_VPA_vs_Scramble_CTRL", "ASO_CTRL_vs_Scramble_CTRL",
  ...
  if (!file.exists(csv)) { cat(sprintf("%-35s: CSV not found\n", cn)); next }
  df <- read.csv(csv)
  hits <- df[!is.na(df$SYMBOL) & df$SYMBOL == "SIGMAR1", ]
  if (nrow(hits) > 0) {
    ...
    cat(sprintf("    coords: %s:%d-%d  annotation: %s  distanceToTSS: %d\n",
                best$seqnames, best$start, best$end, best$annotation, best$distanceToTSS))
  } else {
