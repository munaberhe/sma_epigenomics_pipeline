...
LOCI <- list(
  list(name="HUWE1", chr="chrX", start=53653253, end=53663852,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="HUWE1 intron (p=4.29e-17, E3 ubiquitin ligase, neural)",
       annotation="Intron | hypo | +24.3% | amplified by VPA",
       gene_start=53636829, gene_end=53743950, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="ROCK1", chr="chr18", start=20968790, end=20979089,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="ROCK1 intron (p=4.97e-23, Rho kinase, motor neuron survival)",
       annotation="Intron | hypo | +21.3% | amplified by VPA",
       gene_start=20240000, gene_end=21380000, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="TSHZ1", chr="chr18", start=75209390, end=75220289,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="TSHZ1 intron (p=5.75e-21, motor neuron TF, Caubit 2019)",
       annotation="Intron | hypo | +27.0% | amplified by VPA",
       gene_start=75158000, gene_end=75292000, strand="-",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="CD38", chr="chr4", start=15782464, end=15792763,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="CD38 intron (p=8.66e-06, NAD+ metabolism, neurodegeneration)",
       annotation="Intron | hypo | +27.5% | amplified by VPA",
       gene_start=15779274, gene_end=15851878, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="CACNG5", chr="chr17", start=66906154, end=66916753,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="CACNG5 intergenic (p=2.25e-17, AMPA receptor, VPA reverses ASO)",
       annotation="Intergenic | hypo | +24.9% | VPA reverses ASO hypermethylation",
       gene_start=66835117, gene_end=66894751, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="CACNG4", chr="chr17", start=66992554, end=67003153,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="CACNG4 intron (p=6.29e-10, AMPA receptor, combination-exclusive)",
       annotation="Intron | hypo | +26.1% | combination-exclusive",
       gene_start=66964707, gene_end=67033398, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="CACNG1", chr="chr17", start=67074454, end=67084753,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="CACNG1 intron (p=5.82e-17, AMPA receptor, combination-exclusive)",
       annotation="Intron | hypo | +20.2% | combination-exclusive",
       gene_start=67044554, gene_end=67056797, strand="-",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
  list(name="PRKCA", chr="chr17", start=66306454, end=66316753,
       contrast="ASO_VPA_vs_Scramble_CTRL",
       label="PRKCA intron (p=3.31e-14, protein kinase C, combination-exclusive)",
       annotation="Intron | hypo | +22.7% | combination-exclusive",
       gene_start=66302613, gene_end=66810743, strand="+",
       exons=data.frame(label=character(0), start=numeric(0),
                        end=numeric(0), is_target=logical(0))),
...
plot_one <- function(meth_a, meth_b, ct, locus, dmrs=NULL) {
  ...
  plotLocalMethylationProfile(
    ...
    windowSize       = WIN_SIZE,
    context          = "CG",
    col              = c(COLS[ct$cond_a], COLS[ct$cond_b], "#1F3A5F", "#6B7280", "#E31A1C"),
  ...
  mtext(strand_sym, side=3, line=1.0, cex=0.7, col="#1B4F8A", adj=0, font=2)
}
