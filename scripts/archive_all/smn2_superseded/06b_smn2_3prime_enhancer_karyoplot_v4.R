...
ENH_COL <- "#E69F00"; CGI_COL <- "#A8D5E2"
GENE_COL <- "#F5C16C"; GENE_BORDER <- "#8B6914"
...
message("building gene models...")
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene

draw_genes_manual <- function(kp, r0, r1) {
  ...
  if (length(win_genes)==0) { message("  no genes in window"); return(invisible(NULL)) }
  ...
  kpSegments(kp, chr=as.character(seqnames(win_genes)),
             ...
             y0=0.5, y1=0.5, col=GENE_BORDER, lwd=1.2, r0=r0, r1=r1)
  ...
  if (length(ex_in_win)>0) {
    ...
    if (length(all_exons)>0)
      kpPlotRegions(kp, data=all_exons, col=GENE_COL, border=GENE_BORDER,
                    r0=r0+0.01, r1=r1-0.01, lwd=0.5)
  }
  midpoints <- (pmax(start(win_genes), ZOOM_START) +
                pmin(end(win_genes),   ZOOM_END)) / 2
  kpText(kp, chr=as.character(seqnames(win_genes)), x=midpoints, y=0.92,
         labels=gene_names, col="#1F3A5F", cex=0.75, font=3, r0=r0, r1=r1)
  message("  genes drawn: ", paste(gene_names, collapse=", "))
}

pdf(file.path(OUT_DIR, "SMN2_3prime_enhancer_karyoplot_v4.pdf"), width=12, height=9)

pp <- getDefaultPlotParams(plot.type=2)
pp$leftmargin <- 0.20; pp$rightmargin <- 0.04
pp$topmargin <- 40; pp$bottommargin <- 50
pp$ideogramheight <- 20; pp$data1height <- 460
pp$data2height <- 60; pp$data1inmargin <- 3; pp$data2inmargin <- 3
...
GENE_R0 <- 0.89; GENE_R1 <- 1.00
CGI_R0  <- 0.84; CGI_R1  <- 0.87
ENH_R0  <- 0.79; ENH_R1  <- 0.82
CCRE_R0 <- 0.74; CCRE_R1 <- 0.77

gd <- tryCatch({
  g <- makeGenesDataFromTxDb(txdb=txdb, karyoplot=kp,
                             plot.transcripts=TRUE,
                             plot.transcripts.structure=TRUE)
  g <- addGeneNames(g, orgDb=org.Hs.eg.db)
  mergeTranscripts(g)
}, error=function(e) { message("makeGenesDataFromTxDb failed: ", e$message); NULL })

if (!is.null(gd)) {
  kpPlotGenes(kp, data=gd, r0=GENE_R0, r1=GENE_R1,
              gene.name.cex=0.75, add.gene.names=TRUE,
              coding.exons.col=GENE_COL, coding.exons.border.col=GENE_BORDER,
              non.coding.exons.col=GENE_COL, non.coding.exons.border.col=GENE_BORDER,
              introns.col=GENE_BORDER, marks.col=GENE_BORDER)
} else {
  draw_genes_manual(kp, r0=GENE_R0, r1=GENE_R1)
}
kpAddLabels(kp, "Genes", r0=GENE_R0, r1=GENE_R1, side="left", cex=0.75)

if (length(cgi)>0) kpPlotRegions(kp, data=cgi, col=CGI_COL, border=NA, r0=CGI_R0, r1=CGI_R1)
kpAddLabels(kp, "CpG islands", r0=CGI_R0, r1=CGI_R1, side="left", cex=0.75)

if (length(enh)>0) kpPlotRegions(kp, data=enh, col=ENH_COL, border=ENH_COL, r0=ENH_R0, r1=ENH_R1)
kpAddLabels(kp, "H9 enhancers", r0=ENH_R0, r1=ENH_R1, side="left", cex=0.75)
...
cCRE_enh <- cCRE_df[grepl("ELS",cCRE_df$type) & cCRE_df$chr==ZOOM_CHR &
...
if (nrow(cCRE_enh)>0) {
  ...
  kpPlotRegions(kp, data=cCRE_gr, col="#56B4E9", border="#56B4E9", r0=CCRE_R0, r1=CCRE_R1)
}
kpAddLabels(kp, "ENCODE cCREs", r0=CCRE_R0, r1=CCRE_R1, side="left", cex=0.75)
...
for (i in seq_along(CONDITIONS)) {
  ...
  kpAxis(kp, ymin=0, ymax=1, r0=r0, r1=r1, side=2, cex=0.5, numticks=3)
...
if (file.exists(dmr_csv)) {
  ...
  dmrs <- dmrs[dmrs$seqnames==ZOOM_CHR & dmrs$end>=ZOOM_START & dmrs$start<=ZOOM_END,]
  if (nrow(dmrs)>0) {
    ...
    kpAddLabels(kp, "DMRs", data.panel=2, r0=0.1, r1=0.9, side="left", cex=0.7)
  }
}

dev.off()
message("saved: SMN2_3prime_enhancer_karyoplot_v4.pdf")
