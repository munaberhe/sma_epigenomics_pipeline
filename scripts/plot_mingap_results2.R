...
key_cols <- setNames(
  ...
  levels(df_long$key))
# Long-dash for scrambled so the pattern reads clearly at print size
key_lty <- setNames(
  c("solid","longdash","solid","longdash","solid","longdash"),
  levels(df_long$key))
key_shp <- setNames(
  c(15, 0,  18, 5,  17, 2),
  levels(df_long$key))

# 5% x offset so scrambled dashes are clearly separated from real
df_long$x_plot <- ifelse(df_long$series == "real",
                          df_long$mingap_num * 0.95,
                          df_long$mingap_num * 1.05)
...
p <- ggplot(df_long,
  ...
  # FREE y-scales per minSize panel — each panel auto-scales independently
  # so series with smaller counts aren't squashed by larger ones
