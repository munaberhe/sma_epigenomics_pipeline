...
INNER_WORKERS <- 8
BPPARAM_FAST  <- MulticoreParam(workers = INNER_WORKERS)
...
get_thresholds <- function(method, strict, ws=300) {
  if (!strict) {
    list(pval=0.05, minCpG=1, minDiff=0.1,
         minSize=if (method == "bins") max(1, ws) else 1,
...
run_dmrs_one <- function(treat, ctrl, method, ws, kernel="triangular",
                         strict=FALSE, region) {
  th <- get_thresholds(method, strict, ws)
  # timeout after 30 minutes to prevent infinite merge loops
...
run_dmrs_pair_parallel <- function(treat_real, ctrl_real, treat_scr, ctrl_scr,
  ...
  job_real <- mcparallel({
    s <- Sys.time()
    out <- run_dmrs_one(treat_real, ctrl_real, method, ws, kernel, strict, region)
    list(out=out, t=as.numeric(difftime(Sys.time(), s, units="secs")))
  })
  job_scr <- mcparallel({
    s <- Sys.time()
    out <- run_dmrs_one(treat_scr, ctrl_scr, method, ws, kernel, strict, region)
    list(out=out, t=as.numeric(difftime(Sys.time(), s, units="secs")))
  })
  res <- mccollect(list(job_real, job_scr))
  res_real <- res[[1]]
  res_scr  <- res[[2]]
  if (is.null(res_real)) res_real <- list(out=structure(list(), class="dmr_error",
                                          message="child process died"), t=NA_real_)
  if (is.null(res_scr))  res_scr  <- list(out=structure(list(), class="dmr_error",
                                          message="child process died"), t=NA_real_)
  list(real=res_real$out, scr=res_scr$out, t_real=res_real$t,
       t_scr=res_scr$t, t_wall=as.numeric(difftime(Sys.time(), t_start, units="secs")))
}
...
run_benchmark_loop <- function(aso_vpa, aso_ctrl, aso_vpa_scr, aso_ctrl_scr, null_name) {
  ...
  for (method in METHODS) {
    for (ws in WINDOW_SIZES) {
      ...
      for (strict in MODES) {
        ...
        message("  Running real + scrambled in parallel...")
        ...
        message("  Cell wallclock (parallel): ", round(pair$t_wall), "s")
  ...
  message("Checkpoint kept at: ", ckpt_file)
}
